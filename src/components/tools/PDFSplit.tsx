...
export const PDFSplit = () => {
  ...
  const onFiles = async (files: File[]) => {
    ...
    try {
      ...
      toast({ title: "Invalid PDF", description: e?.message || "Could not read PDF" });
    }
  };
  ...
  const extract = async () => {
    ...
    try {
      ...
      downloadBlob(bytes, `split_${file.name}`);
      toast({ title: "Pages extracted", description: `${indices.length} page(s) saved` });
    } catch (e: any) {
      toast({ title: "Failed to split", description: e?.message || "Unknown error" });
    }
  };
  ...
  return (
    <section className="space-y-6">
      <header className="space-y-2">
        <h2 className="text-2xl font-semibold text-foreground">PDF Split</h2>
        <p className="text-muted-foreground">Extract specific pages from your PDF.</p>
      </header>

      {!file ? (
        <FileUpload onFiles={onFiles} accept={{ "application/pdf": [".pdf"] }} title="Upload PDF" description="Drop a PDF here or browse" />
      ) : (
        <div className="space-y-4">
          <div className="flex items-end gap-3 flex-wrap">
            <div className="space-y-2">
              ...
              {pageCount && (
                <p className="text-xs text-muted-foreground">Total pages: {pageCount}</p>
              )}
            </div>
            <Button onClick={extract}>Extract Selection</Button>
